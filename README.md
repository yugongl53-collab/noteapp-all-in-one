...
> 当前状态：M6 已完成。用户可以维护灵感、学期日程、准点提醒和事件池，使用可恢复的番茄钟，并手动录入实际时间后查看日周统计；JSON 备份仍按 M7 推进。
...
以下是 MVP 核心能力。除 M7 的 JSON 备份与恢复外，其余核心业务流程已经可用：
...
- 待 M7 实现：通过手动导入、导出明文 JSON 备份本地数据。
