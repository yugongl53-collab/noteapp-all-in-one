...
> 当前状态：M3 已完成。除 M2 灵感闭环外，用户可以管理学期、普通事件和课程，在课表或本周事件流中查看同一份计划数据；系统提醒调度、番茄钟、统计和备份仍按 M4 至 M7 推进。
...
以下是 MVP 核心能力。灵感和学期日程流程已经可用，其余业务页面仍按 M4 至 M7 逐步接入：
...
日期采用 `YYYY-MM-DD`，时间采用 `HH:mm`。重叠配置会要求确认但允许保存；启用提醒只保存配置，Android 权限和系统调度将在 M4 接入。
