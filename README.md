...
> 当前状态：M1 数据层与公共规则已实现。应用已具备六类 Room 实体与 DAO、设置及活动番茄钟 DataStore、学期/日程展开规则和时间重叠校验；业务页面仍按 M2 至 M7 里程碑逐步接入。
...
以下是 MVP 目标能力；当前用户界面仍是 M0 导航占位页，已实现的 M1 数据与公共规则用于后续里程碑接入：
